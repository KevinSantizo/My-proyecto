 ...
 <div>
        <v-toolbar   flat text app class="indigo" dark height="57">
          ...
          <v-layout row wrap>
            <v-flex xs12 md12>
              ...
              <v-row justify="center" align="center">
                <v-icon color="white" size="15">mdi-calendar</v-icon><span class="font-weight-bold caption" > {{ this.dayss[new Date().getDay() ]}},{{ new Date().getDate()}} de {{  this.months[new Date().getMonth()] }} {{ new Date().getFullYear() }}</span>
              </v-row>
            </v-flex>
          </v-layout>
          <v-divider inset class="transparent" vertical></v-divider>
              ...
              <v-sheet class="overflow-y-auto transparent" max-height="650" >
              ...
              <template>
                <v-layout row wrap >
                  <v-flex  v-for="(field, i ) in fields" :key="i" class="ma-1 pa-2" style="width: 250px !important">
                    <v-hover >
                      <v-card class="reserve"  color="grey lighten-4"  gradient="to top right, rgba(100,115,201,.33), rgba(25,32,72,.7)">
...
export default {
  data: () => ({
    ...
    methods: {
       getFields() {
      const path = 'http://192.168.1.20:8000/sport/fields/'
